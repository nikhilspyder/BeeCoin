...
public class Beecoin {
	...
	private final static int MINERS_NUM = 3;
	...
	private static final int MINERS_PER_SHARD = 3; // each shard must be in charged of by at least two Miners
	...
	private static void printStats(long totalTime) {
		...
		System.out.println("\nAll mined coins: " + MAX_ITERATIONS * MINING_REWARDS);
...
//			retreiveVerifiedTxions(nextToBeConfirmed[i]);
	...
	public static void simulateAtransaction() {
		...
		double isBalance = getBalance(sender);
		double amount = ((int) (isBalance * Math.random() * 100)) / 100.0;
		String recepient = getRecepient(sender);
		transactions.add(new Transaction(sender, recepient, amount,false));
	}
